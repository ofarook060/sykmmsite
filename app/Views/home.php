...
<head>
    ...
    <style>
        ...
        body { font-family: 'Noto Sans Myanmar', sans-serif; margin: 0; background: #F8F6EE; color: #333; line-height: 1.6; }
        ...
        .hero h1 { margin: 0; font-size: 2.5rem; color: #D4AF37; font-family: 'Noto Serif Myanmar', serif; }
        ...
        .search-form input { flex: 1; padding: 12px; border: 1px solid #ddd; border-radius: 8px; font-size: 1rem; min-width: 200px; font-family: 'Noto Sans Myanmar', sans-serif; }
        .search-form button { padding: 12px 25px; background: #D4AF37; color: #032F2E; border: none; border-radius: 8px; font-size: 1rem; cursor: pointer; font-weight: bold; font-family: 'Noto Sans Myanmar', sans-serif; }
        .search-form button:hover { background: #A87C17; }

        .container { max-width: 1200px; margin: 0 auto; padding: 0 20px; }
        .section-title { text-align: center; margin: 40px 0 20px 0; font-size: 2rem; position: relative; color: #032F2E; font-family: 'Noto Serif Myanmar', serif; }
        ...
        .card-title { font-size: 1.25rem; margin: 0 0 10px 0; color: #032F2E; font-family: 'Noto Serif Myanmar', serif; }
        ...
        .btn-view { display: block; text-align: center; padding: 10px; background: #F8F6EE; color: #D4AF37; border: 1px solid #D4AF37; text-decoration: none; border-radius: 8px; font-weight: bold; margin-top: auto; font-family: 'Noto Sans Myanmar', sans-serif; }
...
<body>
    ...
    <div class="search-container">
        <form action="<?= base_url('/') ?>" method="get" class="search-form">
            ...
            <!-- 1. CHANGED INPUT FROM NUMBER TO TEXT TO PERMIT MYANMAR NUMBERS/CHARACTERS -->
            <input type="text" name="price" placeholder="Maximum Price (e.g. ၁၅၀၀၀၀, 150000)..." value="<?= esc($old_price ?? '') ?>">
            <button type="submit">Filter Listings</button>
            <?php if (!empty($old_location) || !empty($old_price)): ?>
                <a href="<?= base_url('/') ?>" style="padding:12px; color:#dc3545; text-decoration:none;">Clear</a>
            <?php endif; ?>
        </form>
    </div>
                ...
                <?php foreach ($properties as $prop): ?>
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <!-- 3. STRIPPED FORCED FORWARD $ SYMBOL & FLOAT CASTS FROM FLEXIBLE PRICE TEXT -->
                            <div class="card-price"><?= !empty($prop['price']) ? esc($prop['price']) : 'Contact for Price' ?></div>
                            <div class="card-meta">📍 <?= !empty($prop['location']) ? esc($prop['location']) : 'N/A' ?></div>
                            <a href="<?= base_url('properties/view/' . $prop['id']) ?>" class="btn-view">View Details</a>
                        </div>
                    </div>
                <?php endforeach; ?>
            <?php endif; ?>
                ...
                <?php foreach ($latest_posts as $post): ?>
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <a href="<?= base_url('post/' . $post['id']) ?>" class="btn-view">Read Post</a>
                        </div>
                    </div>
                <?php endforeach; ?>
            <?php endif; ?>
